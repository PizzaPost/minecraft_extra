    ...
    private Pair<BlockPos, RegistryEntry<Biome>> findNearestBiome(PlayerEntity player, Identifier targetBiomeId) {
        ...
        Predicate<RegistryEntry<Biome>> biomePredicate = entry -> {
            return entry.getKey().map(key ->
                    key.getValue().equals(targetBiomeId)).orElse(false);};
        ...
        return targetWorld.locateBiome(biomePredicate, searchCenter, dimensionKey == World.NETHER ? 6000 : dimensionKey == World.END ? 10000 : 20000,
                32, dimensionKey == World.NETHER ? 128 : 64);
    }
        ...
        if (!world.isClient()) {
            ...
            if (result != null) {
                ...
                LodestoneTrackerComponent lodestoneTrackerComponent = new LodestoneTrackerComponent(
                        Optional.of(GlobalPos.create(world.getRegistryKey(), biomePos)), false);
                ...
                var changes = ComponentChanges.builder()
                        .add(DataComponentTypes.ITEM_NAME, Text.literal(formattedBiomeId))
                        .build();
