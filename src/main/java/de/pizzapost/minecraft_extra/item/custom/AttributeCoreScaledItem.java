...
public class AttributeCoreScaledItem extends Item {
    ...
    public static void checkAndResetAttributes(ServerPlayerEntity player) {
        ...
        if (!hasItem) {
            double currentSize = player.getAttributeInstance(EntityAttributes.GENERIC_SCALE).getValue();
            double newSize = currentSize + (0.01*(1-currentSize));
            player.getAttributeInstance(EntityAttributes.GENERIC_SCALE).setBaseValue(newSize);
            player.getAttributeInstance(EntityAttributes.PLAYER_BLOCK_BREAK_SPEED).setBaseValue(newSize);
            double newBlockInteractionRange = 4.5*newSize;
            player.getAttributeInstance(EntityAttributes.PLAYER_BLOCK_INTERACTION_RANGE).setBaseValue(newBlockInteractionRange);
            double newArmor = newSize-1;
            player.getAttributeInstance(EntityAttributes.GENERIC_ARMOR).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_ARMOR_TOUGHNESS).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_ATTACK_KNOCKBACK).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_OXYGEN_BONUS).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_WATER_MOVEMENT_EFFICIENCY).setBaseValue(newArmor);
            player.getAttributeInstance(EntityAttributes.GENERIC_ATTACK_DAMAGE).setBaseValue(newSize);
            double newAttackSpeed = -((6.619)/(1-1.812921774133562*Math.pow(e,(0.381386290729889*newSize))));
            player.getAttributeInstance(EntityAttributes.GENERIC_ATTACK_SPEED).setBaseValue(newAttackSpeed);
            double newFallDamageMultiplier = -((2.098)/(1 - 2.032837516048408*Math.pow(e,(0.420730071899163*newSize))));
            player.getAttributeInstance(EntityAttributes.GENERIC_FALL_DAMAGE_MULTIPLIER).setBaseValue(newFallDamageMultiplier);
            ...
            player.getAttributeInstance(EntityAttributes.GENERIC_JUMP_STRENGTH).setBaseValue(newJumpStrength);
            double newMovementSpeed = -0.001595060599008*Math.pow(newSize, 2) + 0.100132039217541*newSize + 0.0015;
            player.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).setBaseValue(newMovementSpeed);
            double newSafeFallDistance = 3*Math.pow(newSize, 0.829);
            player.getAttributeInstance(EntityAttributes.GENERIC_SAFE_FALL_DISTANCE).setBaseValue(newSafeFallDistance);
            ...
            player.getAttributeInstance(EntityAttributes.GENERIC_STEP_HEIGHT).setBaseValue(newStepHeight);
            double newEntityInteractionRange = 3*newSize;
            player.getAttributeInstance(EntityAttributes.PLAYER_ENTITY_INTERACTION_RANGE).setBaseValue(newEntityInteractionRange);
        }
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (player instanceof ServerPlayerEntity serverPlayer) {
            ...
            serverPlayer.getAttributeInstance(EntityAttributes.GENERIC_ATTACK_DAMAGE).setBaseValue(newSize);
            double newAttackSpeed = -((6.619)/(1-1.812921774133562*Math.pow(e,(0.381386290729889*newSize))));
