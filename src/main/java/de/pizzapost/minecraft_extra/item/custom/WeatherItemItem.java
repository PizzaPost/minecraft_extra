        ...
        if (!world.isClient()) {
            ...
            if (server != null) {
                ...
                if (overworld != null) {
                    ...
                    if (random==0) {
                        overworld.setWeather(ServerWorld.CLEAR_WEATHER_DURATION_PROVIDER.get(overworld.getRandom()),
                                0, false, false);
                        ...
                        overworld.setWeather(0,
                                ServerWorld.RAIN_WEATHER_DURATION_PROVIDER.get(overworld.getRandom()),
                                true, false);
                        if (player instanceof ServerPlayerEntity serverPlayer) {
                            Identifier advancementId = Identifier.of(MinecraftExtra.MOD_ID, "weather");
                            AdvancementEntry advancement = serverPlayer.getServer().getAdvancementLoader().get(advancementId);
                        ...
                        overworld.setWeather(0,
                                ServerWorld.THUNDER_WEATHER_DURATION_PROVIDER.get(overworld.getRandom()),
                                true, true);
                        if (player instanceof ServerPlayerEntity serverPlayer) {
                            Identifier advancementId = Identifier.of(MinecraftExtra.MOD_ID, "weather");
                            AdvancementEntry advancement = serverPlayer.getServer().getAdvancementLoader().get(advancementId);
