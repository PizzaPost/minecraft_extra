...
public class IcebombEntity extends ThrownItemEntity {
    ...
    public void tick() {
        ...
        if (!this.getEntityWorld().isClient()) {
            ...
            if (this.getEntityWorld().getBlockState(currentPos).isOf(Blocks.WATER)) {
                int maxRadius = 12;
                int waveDelay = 5;
                if (animationTick == 0) {
                    BlockPos hitPos = this.getBlockPos();
                    if (this.getEntityWorld().getBlockState(hitPos).isOf(Blocks.WATER)) {
                    ...
                    for (int x = -maxRadius; x <= maxRadius; x++) {
                        for (int y = -maxRadius; y <= maxRadius; y++) {
                            for (int z = -maxRadius; z <= maxRadius; z++) {
                                ...
                                if (distance <= maxRadius && this.getEntityWorld().getBlockState(targetPos).isOf(Blocks.WATER)) {
